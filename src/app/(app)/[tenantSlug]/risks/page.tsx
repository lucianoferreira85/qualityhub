...
import { ShieldAlert, FolderKanban, User, Plus } from "lucide-react";
...
const CSV_COLUMNS: CsvColumn<RiskItem>[] = [
  ...
  { key: "category", label: "Categoria", formatter: (v) => CATEGORY_LABELS[String(v)] || String(v) },
...
const TABLE_COLUMNS: Column<RiskItem>[] = [
  { key: "code", label: "Código", sortable: true, className: "font-mono", render: (r) => (
    <span className="text-foreground-tertiary font-mono">{r.code}</span>
  )},
  { key: "title", label: "Título", sortable: true, render: (r) => (
    <span className="font-medium text-foreground-primary line-clamp-1">{r.title}</span>
  )},
  { key: "riskLevel", label: "Nível", render: (r) => (
    <Badge variant={LEVEL_COLORS[r.riskLevel] || ""}>{getRiskLevelLabel(r.riskLevel)}</Badge>
  )},
  { key: "category", label: "Categoria", render: (r) => (
    <span className="text-foreground-secondary">{CATEGORY_LABELS[r.category] || r.category}</span>
  )},
  { key: "status", label: "Status", render: (r) => <StatusBadge status={r.status} type="status" /> },
  { key: "responsible", label: "Responsável", render: (r) => (
    <span className="text-foreground-secondary">{r.responsible?.name?.split(" ")[0] || "—"}</span>
  )},
...
export default function RisksPage() {
  ...
  const [view, setView] = useViewPreference("risks");
  ...
  const hasActiveFilters = !!(filterLevel || filterStatus || filterCategory);
  ...
  return (
    <div className="space-y-6">
      ...
      <FilterBar
        ...
        filters={[
          { key: "level", options: RISK_LEVELS, value: filterLevel },
          { key: "status", options: RISK_STATUSES, value: filterStatus },
          { key: "category", options: RISK_CATEGORIES, value: filterCategory },
        ]}
      ...
      {view === "cards" ? (
        <>
          {loading ? (
            ...
            <EmptyState
              icon={ShieldAlert}
              title={hasActiveFilters || search ? "Nenhum risco encontrado" : "Nenhum risco registrado"}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filtered.map((risk) => (
                <Link key={risk.id} href={`/${tenant.slug}/risks/${risk.id}`}>
                  <Card className="cursor-pointer hover:shadow-card-glow transition-shadow h-full">
                    <CardContent className="p-5">
                      <div className="flex items-start justify-between gap-3 mb-3">
                        <div className="min-w-0">
                          <p className="text-caption-1 text-foreground-tertiary font-mono">{risk.code}</p>
                          <h3 className="text-body-1 font-medium text-foreground-primary mt-0.5 line-clamp-2">{risk.title}</h3>
                        </div>
                      ...
                      <div className="space-y-2 mb-3">
                        ...
                        <div className="flex items-center gap-1.5 text-body-2 text-foreground-secondary">
                          ...
                          <span className="text-foreground-tertiary">({risk.probability}×{risk.impact})</span>
                        </div>
                      </div>
        ...
        <DataTable
          ...
          data={filtered}
          loading={loading}
          onRowClick={(r) => router.push(`/${tenant.slug}/risks/${r.id}`)}
          emptyMessage={hasActiveFilters || search ? "Nenhum risco encontrado" : "Nenhum risco registrado"}
